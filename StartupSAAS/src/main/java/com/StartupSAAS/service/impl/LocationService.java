...
public class LocationService {
    ...
    public PostOffice resolvePostOffice(EmployeeRequest req) {
        ...
        Division division = divisionRepository.findByNameAndCountry(
                req.getDivision(), country
        ).orElseThrow(() -> new RuntimeException("Division not found"));

        District district = districtRepository.findByNameAndDivision(
                req.getDistrict(), division
        ).orElseThrow(() -> new RuntimeException("District not found"));

        PoliceStation ps = policeStationRepository.findByNameAndDistrict(
                req.getPoliceStation(), district
        ).orElseThrow(() -> new RuntimeException("Police station not found"));

        return postOfficeRepository.findByNameAndPoliceStation(
                req.getPostOffice(), ps
